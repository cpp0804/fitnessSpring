...
public class NetSfJsonUtils {
    ...
    static {   
       ...
       MorpherRegistry mr = JSONUtils.getMorpherRegistry();   
        ...
        DateMorpher dm = new DateMorpher(new String[] { "yyyy-MM-dd",   
        		"yyyy-MM-dd HH:mm:ss", "HH:mm:ss", "yyyyMMdd",   
        		"yyyyMMddHHmmss", "HHmmss" }); 
        dm.setUseDefault(true);
        mr.registerMorpher(dm);  */ 
        JSONUtils.getMorpherRegistry().registerMorpher(new DateMorpherEx(new String[] { "yyyy-MM-dd","yyyy-MM-dd HH:mm","yyyy-MM-dd HH:mm:ss", "HH:mm:ss", "yyyyMMdd",   
        		"yyyyMMddHHmmss", "HHmmss"},(Date) null),true);
    }   
    ...
    * @return  
    */  
    public static Object getDtoFromJsonObjStr(String jsonObjStr, Class clazz,List<String> properties) { 
    	try{
    		registerDateMorpher();
    		JSONObject jsonObject = JSONObject.fromObject(jsonObjStr);
    		//  removeEmptyValue(jsonObject, properties);
  ...
  public static void removeEmptyValue(JSONObject jsonObject,List<String> properties){
	  for(String property : properties){
		  if(jsonObject.containsKey(property)&&"".equals(jsonObject.get(property))){			  
			  jsonObject.remove(property);
		  }
	  }
  }
    public static void registerDateMorpher(){
    	JSONUtils.getMorpherRegistry().registerMorpher(new DateMorpherEx(new String[] { "yyyy/MM/dd","yyyy-MM-dd","yyyy-MM-dd HH:mm","yyyy-MM-dd HH:mm:ss", "HH:mm:ss", "yyyyMMdd",   
        		"yyyyMMddHHmmss", "HHmmss"},(Date) null),true);
    	/* //注册器   
        MorpherRegistry mr = JSONUtils.getMorpherRegistry();   
